    ...
    if ~isempty(sac)
        if nsac > 1
            for f = 1:nsac-1
                ...
                fix(f,1) = sac(f,2)+1; % end sac?
                fix(f,2) = sac(f+1,1)-1; % start next sac?
        ...
        for f = 1:size(fix,1)
            ...
            %%% I don't have the statistics toolbox
            v=ver; [installedToolboxes{1:length(v)}] = deal(v.Name);
            if all(ismember(['Financial_Toolbox','Statistics_Toolbox'],installedToolboxes))  % not sure about name
                ...
                fixs1 = fix(f,[4 6]);  % if I don't save it first, it would always be the mean from the first rows?
                fixs2 = fix(f,[5 7]);
                
                fix(f,8) = mean(fixs1(1,~isnan(fixs1)));
                fix(f,9) = mean(fixs2(1,~isnan(fixs2)));                 
